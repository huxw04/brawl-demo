...
目前有四张地图，其中三张用于正式乱斗轮换：

- `stage_b_test_arena`：原有 `1400 × 900 码` 小测试场，供 Lab、Bot 和回归测试使用；
- `large_brawl_01`（四方庭院）：近似对称、中心开阔的基础竞技图；
- `wind_gorge_01`（回风峡谷）：中央山脊与环形侧路，强调绕后、钩爪和弹射；
- `broken_chess_01`（残棋台）：错落房间与长短视线交替的战斗图。
...
1. 更多障碍类型、装饰物、危险区与可交互物；
2. JSON 导入校验报告与地图预览；
3. 程序化生成器输出同一 JSON 格式；
4. 自定义地图内容哈希与联机分发策略。
