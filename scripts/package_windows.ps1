...
$outputDir = Join-Path $projectRoot "dist\BrawlDemo-v0.1"
...
$archivePath = Join-Path $projectRoot "dist\BrawlDemo-v0.1-windows-x86_64.zip"
...
Compress-Archive -Path (Join-Path $outputDir "*") -DestinationPath $archivePath -CompressionLevel Optimal -Force

$packageFiles = Get-ChildItem -LiteralPath $outputDir -File
